/****************************************************************************
** Meta object code from reading C++ file 'webclient.h'
**
** Created: Tue Jun 25 22:06:21 2013
**      by: The Qt Meta Object Compiler version 62 (Qt 4.7.4)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../webclient.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'webclient.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.7.4. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_webClient[] = {

 // content:
       5,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // signals: signature, parameters, type, tag, flags
      16,   11,   10,   10, 0x05,
      54,   43,   10,   10, 0x05,

 // slots: signature, parameters, type, tag, flags
      73,   10,   10,   10, 0x0a,
      88,   10,   10,   10, 0x0a,
     104,   10,   10,   10, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_webClient[] = {
    "webClient\0\0data\0messageArrived(QByteArray)\0"
    "bConnected\0connectState(bool)\0"
    "on_connected()\0on_disconnect()\0"
    "on_readyread()\0"
};

const QMetaObject webClient::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_webClient,
      qt_meta_data_webClient, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &webClient::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *webClient::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *webClient::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_webClient))
        return static_cast<void*>(const_cast< webClient*>(this));
    return QObject::qt_metacast(_clname);
}

int webClient::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: messageArrived((*reinterpret_cast< QByteArray(*)>(_a[1]))); break;
        case 1: connectState((*reinterpret_cast< bool(*)>(_a[1]))); break;
        case 2: on_connected(); break;
        case 3: on_disconnect(); break;
        case 4: on_readyread(); break;
        default: ;
        }
        _id -= 5;
    }
    return _id;
}

// SIGNAL 0
void webClient::messageArrived(QByteArray _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void webClient::connectState(bool _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}
QT_END_MOC_NAMESPACE
